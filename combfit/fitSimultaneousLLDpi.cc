...
ROOT::Fit::FitResult
fitSimultaneousLL(TGraphAsymmErrors *gSC, TGraphAsymmErrors *gOC,
                  double radiusFirst = 0.97, double radiusSecond = 2.52,
                  double weightFirst = 0.66, double maxRange = 300.,
...
void fitSimultaneousLLDpi(const char * inFileName, const char * oFileName, int nIter) {
  ...
  for (auto uncType : {"tot", "stat"}) {
    ...
    for (int iIter = 0; iIter < nIter; iIter++){
      ...
      ROOT::Fit::FitResult result = fitSimultaneousLL(gSC, gOC, radii1[iRadius], radii2[iRadius], weights1[iRadius], fitRanges[iFitRange][1]);
      const double *params = result.GetParams();
      const double *parErrors = result.GetErrors();
