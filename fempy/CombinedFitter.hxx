...
#define DEBUG_COMBFIT(msg) \
    std::cout << "\033[90m" << __FILE__ << "::" << __LINE__ << "\033[0m" << ": " << msg << std::endl
#else
#define DEBUG_COMBFIT(msg)
#endif
...
class GlobalChi2 {
   ...
   public:
    ...
    void SetChi2Function(ROOT::Math::IMultiGenFunction &func, int idx) { fChi2Functions[idx] = &func; }
...
class CombinedFitter {
   public:
    ...
    TH1D *SampledHisto(TH1D *histo, double upprange, int itry) {
        ...
        TH1D *sampledHisto = new TH1D(Form("hTry_%i", itry), "", nBinsFitCF, histo->GetBinLowEdge(1), upprange);
    ...
    void SetBootstrapHistosFeatures() {
        for (int iModel = 0; iModel < fModels.size(); iModel++) {
            std::vector<std::tuple<double, double, double>> modelBootPars =
                this->fModels[iModel].GetBootstrapHistosFeatures();
    ...
    std::vector<TH1D *> CombinedFitBootstrap(int ntries, bool difference) {
        ...
        for (int iPar = 0; iPar < this->fInitPars.size(); iPar++) {
            ...
            if (parName.find("boot") != std::string::npos) {
                ...
                if (originalFitResult.IsParameterFixed(iPar)) {
                    ...
                    hBTHistos.push_back(new TH1D(parName.c_str(), parName.c_str(), 100000, histoMean - 1 * histoBound,
                                                 histoMean + 1 * histoBound));
                } else {
                    double histoBound = originalFitResult.ParError(iPar);
                    hBTHistos.push_back(new TH1D(parName.c_str(), parName.c_str(), 200000, histoMean - 2 * histoBound,
                                                 histoMean + 2 * histoBound));
                }
            }
        }
        ...
        if (difference) {
            for (int iModel = 0; iModel < this->fModels.size(); iModel++) {
                hGenDiffOriginal.push_back(this->fModels[iModel].GetGenuineDifference(
                    static_cast<TH1D *>(this->fModels[iModel].GetFitHisto())));
                ...
                for (int iBin = 0; iBin < nBinsFitCF; iBin++) {
                    ...
                    hBTHistos.push_back(new TH1D(Form("Model%i_Subtraction_%iMeV", iModel, binCenter),
                                                 Form("Model%i_Subtraction_%iMeV", iModel, binCenter), 500,
                                                 binGenuine - 0.1 * binGenuine, binGenuine + 0.1 * binGenuine));
                }
            }
        }
        ...
        for (int iTry = 0; iTry < ntries; iTry++) {
            ...
            if (difference) {
                for (int iModel = 0; iModel < this->fModels.size(); iModel++) {
                    ...
                    for (int iBin = 0; iBin < nBinsFitCF; iBin++) {
                        hBTHistos[this->fInitPars.size() + iModel * nBinsFitCF + iBin]->Fill(
                            hGenDiff->GetBinContent(iBin + 1));
                    }
        ...
        if (difference) {
            for (int iModel = 0; iModel < this->fModels.size(); iModel++) {
                ...
                TH1D *hYields = new TH1D(Form("hYields_Model%i_stat", iModel), Form("hYields_Model%i_stat", iModel),
                                         nBinsFitCF, 0, this->fModels[iModel].GetUppFitRange());
                TH1D *hMeans = new TH1D(Form("hMeans_Model%i_stat", iModel), Form("hMeans_Model%i_stat", iModel),
                                        nBinsFitCF, 0, this->fModels[iModel].GetUppFitRange());
                TH1D *hStdDevs = new TH1D(Form("hStdDevs_Model%i_stat", iModel), Form("hStdDevs_Model%i_stat", iModel),
                                          nBinsFitCF, 0, this->fModels[iModel].GetUppFitRange());
                TH1D *hRelUnc = new TH1D(Form("hRelUnc_Model%i_stat", iModel), Form("hRelUnc_Model%i_stat", iModel),
                                         nBinsFitCF, 0, this->fModels[iModel].GetUppFitRange());

                for (int iBin = 0; iBin < nBinsFitCF; iBin++) {
                    int iHistoIdx = this->fInitPars.size() + iModel * nBinsFitCF + iBin;
                    TF1 *gaus = new TF1("gaus", "gaus", hBTHistos[iHistoIdx]->GetBinLowEdge(1),
                                        hBTHistos[iHistoIdx]->GetBinLowEdge(hBTHistos[iHistoIdx]->GetNbinsX()) +
                                            hBTHistos[iHistoIdx]->GetBinWidth(1));
                    if ((iBin + 1) % 10 == 0) {
                    ...
                    hRelUnc->SetBinContent(iBin + 1, hStdDevs->GetBinContent(iBin + 1) /
                                                         hGenDiffOriginal[iModel]->GetBinContent(iBin + 1));
    ...
    std::vector<TH1D *> CombinedFitDifference() {
        ...
        for (int iModel = 0; iModel < this->fModels.size(); iModel++) {
            fModelFuncts[iModel]->SetFitResult(result, fSingleModelsUniquePars[iModel].data());
            fModels[iModel].SetFitFunction(fModelFuncts[iModel]);
            hDifferences.push_back(
                this->fModels[iModel].GetGenuineDifference(static_cast<TH1D *>(this->fModels[iModel].GetFitHisto())));
        }
    ...
    void SetupGlobalFitter(ROOT::Fit::Fitter *fitter, int ibootstraptry = 0) {
        ...
        fitter->Config().SetParamsSettings(this->fTotalPars,
                                           dummyInit.data());  // number of total parameters, list of init values
        for (int iInitPar = 0; iInitPar < this->fInitPars.size(); iInitPar++) {
            DEBUG_COMBFIT("InitPar" << iInitPar << ": [" << std::get<0>(fInitPars[iInitPar]) << ", "
                                    << std::get<1>(fInitPars[iInitPar]) << ", " << std::get<2>(fInitPars[iInitPar])
                                    << ", " << std::get<3>(fInitPars[iInitPar]) << "]");
            ...
            if (std::get<2>(fInitPars[iInitPar]) >= std::get<3>(fInitPars[iInitPar])) {
                ...
                fitter->Config().ParSettings(iInitPar).SetValue(std::get<1>(fInitPars[iInitPar]));
                fitter->Config().ParSettings(iInitPar).SetLimits(std::get<2>(fInitPars[iInitPar]),
                                                                 std::get<3>(fInitPars[iInitPar]));
            }
        }
    ...
    int GetModelUniqueParams(int imodel) { return this->fSingleModelsUniquePars[imodel].size() - this->fNSharedPars; }

    int GetSharedParams() { return this->fNSharedPars; }

    int GetTotalParams() { return this->fTotalPars; }
   ...
   private:
    void SetTotalParameters() {
        ...
        for (int iModel = 0; iModel < fSingleModelsSharedPars.size(); iModel++) {
            ...
            this->fTotalPars +=
                this->fModels[iModel].GetFitFunction()->GetNpar() - fSingleModelsSharedPars[iModel].size();
        }
        ...
        for (int iModel = 0; iModel < fSingleModelsSharedPars.size(); iModel++) {
            ...
            for (int iPar = 0; iPar < this->fModels[iModel].GetFitFunction()->GetNpar(); iPar++) {
                ...
                if (iPar == this->fSingleModelsSharedPars[iModel][iSharedPar] - 1) {
                    ...
                    DEBUG_COMBFIT("Adding unique parameter: " << iPar + previousModelsUniquePars - iSharedPar
                                                              << " to model " << iModel);
        ...
        for (int iModel = 0; iModel < fModels.size(); iModel++) {
            ...
            for (int iPar = 0; iPar < this->fModels[iModel].GetFitFunction()->GetNpar(); iPar++) {
                ...
                std::tuple<std::string, double, double, double> initPar = {
                    ...
                    this->fModels[iModel].GetFitFunction()->GetParameter(iPar), lowerLim, upperLim};
                DEBUG_COMBFIT("iPar" << iPar << ": ";
                              cout << this->fModels[iModel].GetFitFunction()->GetParName(iPar) << ", ";
                              cout << this->fModels[iModel].GetFitFunction()->GetParameter(iPar) << ", " << lowerLim
                                   << ", " << upperLim);
                if (std::find(fSingleModelsSharedPars[iModel].begin(), fSingleModelsSharedPars[iModel].end(),
                              iPar + 1) != fSingleModelsSharedPars[iModel].end()) {
