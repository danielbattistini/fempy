...
class CorrelationFitter {
   public:
    ...
    void DrawSpline(TVirtualPad *pad, TH1 *hist, std::string name = ";k* (MeV/c);Counts") {
        pad->cd();
    ...
    void DrawSpline(TVirtualPad *pad, TGraph *graph, std::string name = ";k* (MeV/c);Counts") {
        pad->cd();
    ...
    */
    ...
    void Add(TString name, TH1 *hist, std::vector<std::tuple<std::string, double, double, double>> pars,
             std::string addmode, int relweight = 0) {
    ...
    void Add(TString name, TGraph *graph, std::vector<std::tuple<std::string, double, double, double>> pars,
             std::string addmode, int relweight = 0) {
    ...
    void Add(TString name, TSpline3 *spline, std::vector<std::tuple<std::string, double, double, double>> pars,
             std::string addmode, int relweight = 0) {
        DEBUG("Relweight " << relweight);
        this->fRelWeights.push_back(relweight);
    ...
    TF1 *GetBaseline(bool globnorm = 1, bool lambdagen = 0) {
        ...
        // TF1 *fBaseline = new TF1("fBaseline", std::get<0>(functions[this->fFitFuncComps[baselineIdx]]), fFitRangeMin,
        // fFitRangeMax, std::cout << "Range for baseline: " << fFitRangeMin << " " << fFitRangeMax << endl; std::cout
        // << "Range for baseline: " << this->fFitFuncComps[baselineIdx] << endl; std::cout << "Number of parameters: "
        // << std::get<1>(functions[this->fFitFuncComps[baselineIdx]]) << endl;
        TF1 *fBaseline = new TF1("fBaseline", std::get<0>(functions[this->fFitFuncComps[baselineIdx]]), fFitRangeMin,
                                 fFitRangeMax, std::get<1>(functions[this->fFitFuncComps[baselineIdx]]));
        ...
        TF1 *fBaselineGlobNorm = new TF1(
            "fBaseline",
            [&, this, fBaseline, lambdagen, globnorm](double *x, double *par) {
                if (lambdagen) {
                    return this->fLambdaGen * this->fFit->GetParameter(this->fFit->GetNpar() - 1) *
                           fBaseline->Eval(x[0]);
                }
    ...
    std::vector<TF1 *> GetAncestors() {
        ...
        TF1 *fCommonGlobNorm = new TF1(
            "fCommon",
            [&, this, fCommon, commonWeight](double *x, double *par) {
                return 1 +
                       this->fFit->GetParameter(this->fFit->GetNpar() - 1) * commonWeight * (fCommon->Eval(x[0]) - 1);
            },
            fFitRangeMin, fFitRangeMax, 0);
        fAncestorTemplates.push_back(fCommonGlobNorm);

        TF1 *fNonCommonGlobNorm = new TF1(
            "fNonCommon",
            [&, this, fNonCommon, commonWeight](double *x, double *par) {
                return 1 + this->fFit->GetParameter(this->fFit->GetNpar() - 1) * (1 - commonWeight) *
                               (fNonCommon->Eval(x[0]) - 1);
            },
            fFitRangeMin, fFitRangeMax, 0);
    ...
    TF1 *GetGenuine() {
        ...
        // TF1 *fGenuine = new TF1("fGenuine", std::get<0>(functions[this->fFitFuncComps[genuineIdx]]), fFitRangeMin,
        // fFitRangeMax,
        std::cout << "Range for fGenuine: " << fFitRangeMin << " " << fFitRangeMax << endl;
        TF1 *fGenuine = new TF1("fGenuine", std::get<0>(functions[this->fFitFuncComps[genuineIdx]]), fFitRangeMin,
                                fFitRangeMax, std::get<1>(functions[this->fFitFuncComps[genuineIdx]]));
    ...
    TH1D *SaveFreeFixPars() {
        ...
        TH1D *histoFreeFixPars =
            new TH1D("hFreeFixPars", "hFreeFixPars", this->fFit->GetNpar() + 2, 0, this->fFit->GetNpar() + 2);
    ...
    TH1D *SaveFitParsSplitComponents(std::vector<std::string> subcompsmothers, std::vector<int> compstosplit,
                                     std::vector<std::vector<int>> compsnormsidx,
                                     std::vector<std::vector<std::string>> compsnames,
                                     std::vector<std::vector<std::string>> normslabels) {
        if (!this->fFit) {
            throw std::invalid_argument("Fit not performed, component cannot be evaluated!");
        }
        ...
        TH1D *histoAllCompsPars = new TH1D("hAllCompsPars", "hAllCompsPars", this->fFit->GetNpar() + nCompsPars, 0,
                                           this->fFit->GetNpar() + nCompsPars);
            ...
            for (int iComp = 0; iComp < compsnames[iSplitComp].size(); iComp++) {
                ...
                if (compsnormsidx[iSplitComp][iComp] > 0) {
                    DEBUG(std::setw(30) << "[Norm] Filling bin no. " << nBinNewCompPar + 1;
                          cout << " with "
                               << histoAllCompsPars->GetBinContent(startCompPar + compsnormsidx[iSplitComp][iComp] - 1);
                          cout << ", copied from bin no. " << startCompPar + compsnormsidx[iSplitComp][iComp] - 1);

                    histoAllCompsPars->SetBinContent(
                        nBinNewCompPar + 1,
                        histoAllCompsPars->GetBinContent(startCompPar + compsnormsidx[iSplitComp][iComp] - 1));
                }
                if (compsnormsidx[iSplitComp][iComp] < 0) {
                    DEBUG(std::setw(30) << "[Norm] Filling bin no. " << nBinNewCompPar + 1;
                          cout << " with "
                               << 1 - histoAllCompsPars->GetBinContent(startCompPar + compsnormsidx[iSplitComp][iComp] -
                                                                       1);
                          cout << ", complementary of bin no. " << startCompPar + compsnormsidx[iSplitComp][iComp] - 1);
                    histoAllCompsPars->SetBinContent(
                        nBinNewCompPar + 1,
                        1 - histoAllCompsPars->GetBinContent(startCompPar + compsnormsidx[iSplitComp][iComp] - 1));
                }
                if (compsnormsidx[iSplitComp][iComp] == 0) {
                    DEBUG(std::setw(30) << "[Norm] Filling bin no. " << nBinNewCompPar + 1;
                ...
                for (int iCompPar = 0; iCompPar < compPar; iCompPar++) {
                    DEBUG(std::setw(30) << "Filling bin no. " << nBinNewCompPar + iCompPar + 2;
                          cout << " with "
                               << histoAllCompsPars->GetBinContent(startCompPar + iCompPar + previousCompsPars + 1);
                          cout << ", copied from bin no. " << startCompPar + iCompPar + previousCompsPars + 1);
                    histoAllCompsPars->SetBinContent(
                        nBinNewCompPar + iCompPar + 2,
                        histoAllCompsPars->GetBinContent(startCompPar + iCompPar + previousCompsPars + 1));
                    histoAllCompsPars->GetXaxis()->SetBinLabel(
                        nBinNewCompPar + iCompPar + 2,
                        this->fFit->GetParName(startCompPar + iCompPar + previousCompsPars - this->fGlobNorm));
                }
    ...
    TH1D *PullDistribution() {
        ...
        for (int iBin = 0; iBin < this->fFitHist->GetNbinsX(); iBin++) {
            if (this->fFitHist->GetBinCenter(iBin + 1) >= this->fFitRangeMin &&
                ...
                pulls.push_back((this->fFitHist->GetBinContent(iBin + 1) -
                                 this->fFit->Eval(this->fFitHist->GetBinCenter(iBin + 1))) /
    ...
    void SetFitFunction(TF1 *function) { this->fFit = function; }
    ...
    std::vector<TH1D *> BootstrapComponents(int itry, bool fromoutside = false) {
        ...
        for (const auto &comp : fBootstrapHistos) {
            ...
            for (int iSubComp = 0; iSubComp < nSubComps; iSubComp++) {
                ...
                int previousNPar = accumulate(fNPars.begin(), std::next(fNPars.begin(), comp.first + 1),
                                              std::get<3>(funcTuple)[iSubComp] + 1);
                ...
                TF1 *fComp =
                    new TF1(funcName.c_str(), std::get<0>(functions[std::get<2>(funcTuple)[iSubComp]]), lowFitRange,
                            uppFitRange, std::get<1>(functions[std::get<2>(funcTuple)[iSubComp]]));

                for (int iSubCompPar = 0; iSubCompPar < fComp->GetNpar(); iSubCompPar++) {
                    DEBUG("Initializing bootstrap fit subcomp par to "
                          << this->fFit->GetParName(previousNPar + iSubCompPar) << " of value "
                          << this->fFit->GetParameter(previousNPar + iSubCompPar));
                    // 4 because the first three bins of the hFreeFixPars histo are lowfitrange, uppfitrange,
                    // normalization
                ...
                for (int iSubCompPar = 0; iSubCompPar < fComp->GetNpar(); iSubCompPar++) {
                    DEBUG("New value for " << this->fFit->GetParName(previousNPar + iSubCompPar) << ": "
                                           << fComp->GetParameter(iSubCompPar));
    ...
    TH1D *SampledHisto(const TH1D &histo, double upprange, std::string funcname, int itry) {
        ...
        TH1D *sampledHisto = new TH1D(Form("h%s_try%i", funcname.c_str(), itry),
                                      Form("h%s_try%i", funcname.c_str(), itry), nBinsFitCF, 0, upprange);
                    ...
                    if (fFitFuncComps[iTerm].Contains("splinehisto")) {
                        if (fAddModes[iTerm] == "*") {
                            ...
                            if (this->fNPars[iTerm + 1] == 2) {
                                if (normNPar > 0) {
                                    result = (pars[nPar] / pars[normNPar]) *
                                             this->fFitSplines[nSplineComp]->Eval(x[0] - pars[nPar + 1]) * partResult;
                                } else {
                                    result = (pars[nPar]) *
                                             this->fFitSplines[nSplineComp]->Eval(x[0] - pars[nPar + 1]) * partResult;
                                }
                                ...
                                if (normNPar > 0) {
                                    result = (pars[nPar] / pars[normNPar]) *
                                             this->fFitSplines[nSplineComp]->Eval(x[0]) * partResult;
                                } else {
                            ...
                            if (this->fNPars[iTerm + 1] == 2) {
                                if (normNPar > 0) {
                                    result += (pars[nPar] / pars[normNPar]) *
                                              this->fFitSplines[nSplineComp]->Eval(x[0] - pars[nPar + 1]);
                                } else {
                                    result +=
                                        (pars[nPar]) * this->fFitSplines[nSplineComp]->Eval(x[0] - pars[nPar + 1]);
                                }
                            } else {
                                if (normNPar > 0) {
                                    result +=
                                        (pars[nPar] / pars[normNPar]) * this->fFitSplines[nSplineComp]->Eval(x[0]);
                                } else {
    ...
    std::vector<TH1D *> GetDifference(int ntries = 0) {
        ...
        TH1D *hSubtraction = new TH1D("hSubtraction", "hSubtraction", nBinsFitCF, 0,
        ...
        if (ntries > 0) {
            ...
            for (int iBin = 0; iBin < nBinsFitCF; iBin++) {
                ...
                hBinsCFDifferences.push_back(new TH1D(Form("Subtraction_%iMeV", binCenter),
                                                      Form("Subtraction_%iMeV", binCenter), 10000,
                                                      binGenuine - 0.2 * binGenuine, binGenuine + 0.2 * binGenuine));
            }

            // loop over the tries
            for (int iTry = 0; iTry < ntries; iTry++) {
                // compute CF with gaussian sampling
                TH1D *sampledCF = new TH1D(Form("hTry_%i", iTry), "", nBinsFitCF, this->fFitHist->GetBinLowEdge(1),
                                           this->fFitRangeMax);
            ...
            int index = 0;
            hFitParsBT.erase(
                std::remove_if(hFitParsBT.begin(), hFitParsBT.end(),
            ...
            TH1D *hYields =
                new TH1D("hYields", "hYields", nBinsFitCF, 0, static_cast<int>(nBinsFitCF * fFitHist->GetBinWidth(1)));
            TH1D *hMeans =
                new TH1D("hMeans", "hMeans", nBinsFitCF, 0, static_cast<int>(nBinsFitCF * fFitHist->GetBinWidth(1)));
            TH1D *hStdDevs = new TH1D("hStdDevs", "hStdDevs", nBinsFitCF, 0,
                                      static_cast<int>(nBinsFitCF * fFitHist->GetBinWidth(1)));
            TH1D *hRelUnc =
                new TH1D("hRelUnc", "hRelUnc", nBinsFitCF, 0, static_cast<int>(nBinsFitCF * fFitHist->GetBinWidth(1)));
            for (int iBin = 0; iBin < hBinsCFDifferences.size(); iBin++) {
                TF1 *gaus = new TF1("gaus", "gaus", hBinsCFDifferences[iBin]->GetBinLowEdge(1),
                                    hBinsCFDifferences[iBin]->GetBinLowEdge(hBinsCFDifferences[iBin]->GetNbinsX()) +
                                        hBinsCFDifferences[iBin]->GetBinWidth(1));
                gaus->SetParameter(1,
                                   hBinsCFDifferences[iBin]->GetBinCenter(hBinsCFDifferences[iBin]->GetMaximumBin()));
                ...
                hRelUnc->SetBinContent(iBin + 1,
                                       hStdDevs->GetBinContent(iBin + 1) / hSubtraction->GetBinContent(iBin + 1));
            }
    ...
    void SetBootstrapComp(int icomp, std::vector<TH1D> bootstraphistos, std::vector<TH1D> bootstrapfreefixpars,
                          std::vector<double> bootstrapfitranges,
                          std::vector<std::tuple<double, double, double>> boothistranges) {
        ...
        for (int iBootFeature = 0; iBootFeature < boothistranges.size(); iBootFeature++) {
            cout << std::get<0>(boothistranges[iBootFeature]) << " " << std::get<1>(boothistranges[iBootFeature]) << " "
                 << std::get<2>(boothistranges[iBootFeature]) << endl;
    ...
    TH1D *GetGenuineDifference(TH1D *histo) {
        ...
        for (int iBin = 0; iBin < hGenuineDiff->GetNbinsX(); iBin++) {
            ...
            hGenuineDiff->SetBinContent(iBin + 1, 1 + (histo->GetBinContent(iBin + 1) - this->fFit->Eval(binCenter)) /
    ...
    std::vector<TH1D *> Bootstrap(int ntries) {
        ...
        for (int iTry = 0; iTry < ntries; iTry++) {
            // compute CF with gaussian sampling
            TH1D *sampledCF =
                new TH1D(Form("hTry_%i", iTry), "", nBinsFitCF, this->fFitHist->GetBinLowEdge(1), this->fFitRangeMax);
            for (int iSampledBin = 0; iSampledBin < sampledCF->GetNbinsX(); iSampledBin++) {
                double CFvalue = this->fFitHist->GetBinContent(iSampledBin + 1);
                double CFerror = this->fFitHist->GetBinError(iSampledBin + 1);
                sampledCF->SetBinContent(iSampledBin + 1, gRandom->Gaus(CFvalue, CFerror));
    ...
    double GetChi2NdfManual() {
        ...
        for (int iBin = 0; iBin < this->fFitHist->GetNbinsX(); iBin++) {
            if (this->fFitHist->GetBinError(iBin + 1) != 0) {
                chi2 += ((this->fFit->Eval(this->fFitHist->GetBinCenter(iBin + 1)) -
                          this->fFitHist->GetBinContent(iBin + 1)) *
                         (this->fFit->Eval(this->fFitHist->GetBinCenter(iBin + 1)) -
                          this->fFitHist->GetBinContent(iBin + 1))) /
    ...
    TH1 *GetFitHisto() { return this->fFitHist; }

    double GetLowFitRange() { return this->fFitRangeMin; }
   ...
   private:
    ...
    std::map<int, std::tuple<std::vector<TH1D>, std::vector<TH1D>, std::vector<double>>>
        fBootstrapHistos;  // First element of map: index of component to bootstrap
    ...
    std::vector<std::tuple<double, double, double>>
        fBootHistoParsFeatures;  // nbins, lowedge, uppedge of each parameter which will be bootstrapped
};
