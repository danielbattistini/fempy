...
class DrawFitFuncts {
   public:
    DrawFitFuncts(TH1 *fithist, double drawRangeMin, double drawRangeMax, bool globNorm = 0, int basIdx = -1) {
    ...
    void SetParHist(TH1 *parHist) { this->hParameters = parHist; }

    void SetTotalFitFunc(TF1 *totalFitFunc) { this->fFit = totalFitFunc; }
    ...
    void SetGlobNorm(bool setGlobNorm) { this->fGlobNorm = setGlobNorm; }
    ...
    void AddSplineHisto(TGraph *splinegraph) {
        ...
        DEBUG("Adding TGraph " << splinegraph->GetName() << endl; cout << "Spline at 10 MeV/c " << spline->Eval(10));
        this->fSplines.push_back(spline);
    }

    void EvaluateToBeDrawnComponents(std::vector<bool> onBaseline, std::vector<bool> multNorm,
                                     std::vector<bool> multGlobNorm, std::vector<double> funcshifts, int basIdx = -1,
                                     std::vector<TString> addComps = {""}) {
        ...
        for (int iFunc = 0; iFunc < this->fFitFuncNames.size(); iFunc++) {
            ...
            if (this->fFitFuncNames[iFunc].Contains("spline")) {  // Build shifted spline
                ...
                DEBUG("Loaded shift parameter from histogram. Bin = " << iPar + 1 << "  " << parName << "="
                                                                      << splineShift);

                // Build shifted spline
                TF1 *shiftedSpline = new TF1(
                    this->fFitFuncNames[iFunc],
                ...
                rawComps.push_back(new TF1(this->fFitFuncNames[iFunc],
                                           std::get<0>(functions[this->fFitFuncNames[iFunc]]), fDrawRangeMin,
                                           fDrawRangeMax, std::get<1>(functions[this->fFitFuncNames[iFunc]])));
                DEBUG("Set pars of comp " << iFunc << ", named " << this->fFitFuncNames[iFunc] << ", having "
                                          << rawComps.back()->GetNpar() << " parameters" << endl;
                      cout << "StartPar: " << startPar);
                for (int iPar = 0; iPar < rawComps.back()->GetNpar(); iPar++) {
                    DEBUG("Set par n. " << iPar << " to "
                                        << this->hParameters->GetXaxis()
                                               ->GetLabels()
                                               ->At(startPar + iFunc + iPar + 1 + this->fGlobNorm)
                                               ->GetName();
                          cout << ", bin no. " << startPar + iFunc + iPar + 1 << " bin content: "
                               << this->hParameters->GetBinContent(startPar + iFunc + iPar + 2 + this->fGlobNorm));
                    rawComps.back()->FixParameter(
                        iPar, this->hParameters->GetBinContent(startPar + iFunc + iPar + 2 + this->fGlobNorm));
                }
        ...
        for (int iFunc = 0; iFunc < this->fFitFuncNames.size(); iFunc++) {
            if (multNorm[iFunc]) {
                int normIdx =
                    accumulate(nParsComps.begin(), std::next(nParsComps.begin(), iFunc), 0) + iFunc + this->fGlobNorm;
        ...
        if (addComps[0] != "") {
            for (int iAddComp = 0; iAddComp < addComps.size(); iAddComp++) {
                ...
                TF1 *sumComps = new TF1(
                    "SumComp_" + addComps[iAddComp],
                    [&, this, rawComps, norms, addComps, iAddComp, onBaseline](double *x, double *pars) {
                        double sum = 0.;
                ...
                for (int iFunc = 0; iFunc < onBaseline.size(); iFunc++) {
                    if (addComps[iAddComp].Contains(std::to_string(iFunc))) {
                        ...
                        if (!multGlobNorm[iFunc]) {
                            cout << "Not all components are to be multiplied for the global normalization constant, "
                                    "their sum will not be ";
        ...
        if (basIdx != -1) {
            ...
            bas = new TF1(
                this->fFitFuncNames[basIdx],
                [&, this, rawComps, multNorm, multGlobNorm, basIdx](double *x, double *pars) {
        ...
        for (int iRawComp = 0; iRawComp < rawComps.size(); iRawComp++) {
            ...
            this->fFuncToBeDrawn.push_back(new TF1(
                this->fFitFuncNames[iRawComp],
                ...
                this->fDrawRangeMin, this->fDrawRangeMax, 0));
            DEBUG("Evaluate component: " << this->fFitFuncNames[iRawComp]
                                         << " value@200MeV: " << this->fFuncToBeDrawn.back()->Eval(200));
        }
    ...
    /*
     */
    void Draw(std::vector<TString> legLabels, std::vector<int> colors, std::vector<double> legCoords,
              int linesThickness, double lowRangeUser = 0.0, double uppRangeUser = 1.05,
        ...
        for (int iFuncEval = 0; iFuncEval < fFuncToBeDrawn.size(); iFuncEval++) {
            ...
            DEBUG("idx: " << iFuncEval << " value: " << this->fFuncToBeDrawn[iFuncEval]->Eval(200)
                          << " name: " << fFitFuncNames[iFuncEval]);
        }
