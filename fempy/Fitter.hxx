...
class Fitter {
    ...
    */
    ...
    int Fit() {
        ...
        for(int iFunc=0; iFunc<fFitFunc.size(); iFunc++) {
            this->fFitFuncEval.push_back(new TF1(Form("fComp_%.0f", iFunc), fFitFunc[iFunc], fFitRangeMin, fFitRangeMax, fNPars[iFunc+1]));
            for(int iPar=0; iPar<fNPars[iFunc+1]; iPar++) {
                // cout << "Setting parameter: " << iPar << " at " << this->fFit->GetParameter(setPars + iPar) << endl;
                this->fFitFuncEval.back()->SetParameter(iPar, this->fFit->GetParameter(setPars + iPar));
            }
    ...
    */
    void Draw(TVirtualPad *pad) {
        ...
        gPad->DrawFrame(fFitRangeMin, 0, fFitRangeMax, 1.15,
        ...
        for(int iFunc=0; iFunc<fFitFuncEval.size(); iFunc++) {
            ...
            this->fFitFuncEval[iFunc]->DrawClone("same");
            pad->Modified();
            pad->Update();
        }
        ...
        //int iPar = 0;
        //for(int iFunc=0; iFunc<fFitFunc.size(); iFunc++) {
        //    // cout << fNPars[iFunc] << endl;
        //    TF1 *iComp = new TF1(Form("Comp %.0f", iFunc), fFitFunc[iFunc], fFitRangeMin, fFitRangeMax, fNPars[iFunc+1]);
        //    for(int jPar=0; jPar<fNPars[iFunc+1]; jPar++) {
        //        iComp->FixParameter(jPar, this->fFit->GetParameter(iPar));
        //        // cout << "FIXING PARS" << endl;
        //        iPar++;
        //    }
        //    // cout << "DRAW" << endl;
        //    iComp->SetNpx(300);
        //    iComp->SetLineColor(kBlack);
        //    pad->Modified();
        //    pad->Update();
        //    iComp->Draw("same");
        //    pad->Modified();
        ...
        //TF1 *Try = new TF1("Try", Pol0, fFitRangeMin, fFitRangeMax, 10);
        //int tryPar = 0;
        //for(int iFunc=0; iFunc<fFitFunc.size(); iFunc++) {
        //    // cout << fNPars[iFunc] << endl;
        //    for(int jPar=0; jPar<fNPars[iFunc+1]; jPar++) {
        //        Try->FixParameter(jPar, this->fFit->GetParameter(tryPar));
        //        // cout << "FIXING PARS" << endl;
        //        tryPar++;
        //    }
        //    // cout << "DRAW" << endl;
        //    Try->SetNpx(300);
        //    Try->SetLineColor(kBlack);
        //    // cout << "Eval: " << Try->Eval(0.1) << endl;
        //    pad->Modified();
        //    pad->Update();
        //    Try->Draw("same");
        //    pad->Modified();
        //    pad->Update();
        //    for(int totPar=0; totPar<Try->GetNpar(); totPar++) {
        //        Try->FixParameter(totPar, 0.000);
        //    } 
        //}
        //Try->SetParameter(0,1);
        //Try->SetNpx(300);
        //Try->SetLineColor(kBlue);
        //Try->Draw("same");
