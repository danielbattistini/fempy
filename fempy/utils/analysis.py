...
class Pair:
    def __init__(self, pair):
        if pair == 'DstarPi':
            ...
            self.checks = [
                ('oldpc_dummy', '(light_px**2 + light_py**2)**0.5 > 0.14'),
                # ('oldpc', '(is_oldpcrm == 0) & ((light_eta < 0.8 | light_eta > -0.8)) & ((light_px**2 + light_py**2)**0.5 > 0.14) & ((light_nsigtpc < 3) | light_nsigtpc > -3) & (light_ncrossed > 80) & (light_dcaz < 0.1 | light_dcaz > -0.1) & (light_dcaz < 0.1 | light_dcaz > -0.1)'),
                # ('multDeq1', 'heavy_mult == 1 & is_oldpcrm == 0 & (light_eta < 0.8 | light_eta > -0.8) & (light_px**2 + light_py**2)**0.5 > 0.14 & (light_nsigtpc < 3 | light_nsigtpc > -3) & light_ncrossed > 80 & (light_dcaz < 0.1 | light_dcaz > -0.1) & (light_dcaz < 0.1 | light_dcaz > -0.1)'),
                # ('multDgt1', 'heavy_mult > 1 & is_oldpcrm == 0 & (light_eta < 0.8 | light_eta > -0.8) & (light_px**2 + light_py**2)**0.5 > 0.14 & (light_nsigtpc < 3 | light_nsigtpc > -3) & light_ncrossed > 80 & (light_dcaz < 0.1 | light_dcaz > -0.1) & (light_dcaz < 0.1 | light_dcaz > -0.1)'),
            ]

            # check PID selections
            self.syst_sel = {
